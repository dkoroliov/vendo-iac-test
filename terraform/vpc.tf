...
# Create a VPC
resource "aws_vpc" "vendo-iac_vpc" {
...
resource "aws_internet_gateway" "vendo-iac_igw" {
...
resource "aws_subnet" "public" {
  ...
  cidr_block              = "${cidrsubnet(aws_vpc.vendo-iac_vpc.cidr_block, 8, count.index)}"
...
resource "aws_subnet" "private" {
  ...
  count                   = "${length(var.azs_list)}"
  availability_zone       = "${var.azs_list[count.index]}"
  cidr_block              = "${cidrsubnet(aws_vpc.vendo-iac_vpc.cidr_block, 8, 10 + count.index)}"
...
resource "aws_eip" "nat_eip" {
  count      = "${length(var.azs_list)}"
  vpc        = true
  depends_on = ["aws_internet_gateway.vendo-iac_igw"]
}
...
resource "aws_nat_gateway" "nat" {
  ...
  depends_on     = ["aws_internet_gateway.vendo-iac_igw", "aws_eip.nat_eip"]
...
resource "aws_default_route_table" "public" {
  default_route_table_id = "${aws_vpc.vendo-iac_vpc.main_route_table_id}"
}

# associate route table with the public subnets
resource "aws_route_table_association" "public" {
  count          = "${length(var.azs_list)}"
  route_table_id = "${aws_vpc.vendo-iac_vpc.main_route_table_id}"
  subnet_id      = "${element(aws_subnet.public.*.id, count.index)}"
}
...
resource "aws_route" "internet_access" {
  ...
  gateway_id             = "${aws_internet_gateway.vendo-iac_igw.id}"
}
